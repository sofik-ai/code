...
export class GlobalCompositeBar extends Disposable {
	...
	private toggleAccountsActivity() {
		if (!this.accountsVisibilityPreference) {
			if (this.globalActivityActionBar.length() === 2) { this.globalActivityActionBar.pull(GlobalCompositeBar.ACCOUNTS_ACTION_INDEX); }
			return;
		}
...
export function isAccountsActionVisible(storageService: IStorageService): boolean {
	return false; // Sofik has no editor accounts.
}
