...
function toDefaultAccountConfig(defaultChatAgent: IDefaultChatAgent | undefined): IDefaultAccountConfig {
	if (!defaultChatAgent) {
		return {
			preferredExtensions: [],
			authenticationProvider: { default: { id: '', name: '' }, enterprise: { id: '', name: '' }, enterpriseProviderConfig: '', enterpriseProviderUriSetting: '', scopes: [] },
			entitlementUrl: '', tokenEntitlementUrl: '', mcpRegistryDataUrl: '', managedSettingsUrl: ''
		};
	}
	return {
		preferredExtensions: [
...
export class DefaultAccountService extends Disposable implements IDefaultAccountService {
	...
	constructor(
		...
		if (!productService.defaultChatAgent) { this.initBarrier.open(); }
	}
...
class DefaultAccountProviderContribution extends Disposable implements IWorkbenchContribution {
	...
	constructor(
		...
		super();
		if (!productService.defaultChatAgent) { return; }
